...
generate:
    cargo run -p lutra-cli -- gen-code --lutra-bin-path="crate" --project=. ./generated.rs --no-std
    cargo fmt -p lutra-bin

[working-directory('lutra-bin/src/project')]
generate-no-compile:
    # For when current code does not compile, but we still have an old cli binary
    ../../../target/debug/lutra gen-code --lutra-bin-path="crate" --project=. ./generated.rs --no-std
    cargo fmt -p lutra-bin
