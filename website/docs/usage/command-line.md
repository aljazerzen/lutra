...
!!! note
    ...
    While Lutra (the project) is in a pre-release state, none of the packages are yet published to crates.io. Instead, we use a custom registry at [codeberg.org](https://codeberg.org/lutra/cargo/) to host the packages. This is why we need to use the `--index` flag to tell `cargo` to use this registry.
