...
use lutra_compiler::{
    _bench, _lexer, CheckParams, CompileParams, ProgramRepr, SourceTree, check, compile,
};
...
fn bench_compile(c: &mut Criterion) {
    ...
    group.bench_function("bytecode", |b| {
        b.iter(|| {
            let params = CompileParams::new("main", ProgramRepr::BytecodeLt);
            compile(black_box(&project), &params).unwrap()
        });
    });
    group.bench_function("sql_duckdb", |b| {
        b.iter(|| {
            let params = CompileParams::new("main", ProgramRepr::SqlDuckdb);
            compile(black_box(&project), &params).unwrap()
        });
    });
    group.finish();
