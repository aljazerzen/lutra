...
pub struct Program {
    pub format: crate::string::String,
    pub inner: crate::vec::Vec<u8>,
}
...
mod impls {
    ...
    impl crate::Encode for Program {
        type HeadPtr = ProgramHeadPtr;
        fn encode_head(&self, buf: &mut crate::bytes::BytesMut) -> Self::HeadPtr {
            let format = self.format.encode_head(buf);
            let inner = self.inner.encode_head(buf);
            ProgramHeadPtr { format, inner }
        }
        fn encode_body(&self, head: Self::HeadPtr, buf: &mut crate::bytes::BytesMut) {
            self.format.encode_body(head.format, buf);
            self.inner.encode_body(head.inner, buf);
        }
    }
    #[allow(non_camel_case_types)]
    pub struct ProgramHeadPtr {
        format: <crate::string::String as crate::Encode>::HeadPtr,
        inner: <crate::vec::Vec<u8> as crate::Encode>::HeadPtr,
    }
    impl crate::Layout for Program {
        fn head_size() -> usize {
            128
    ...
    impl crate::Decode for Program {
        fn decode(buf: &[u8]) -> crate::Result<Self> {
            let format = crate::string::String::decode(buf.skip(0))?;
            let inner = crate::vec::Vec::<u8>::decode(buf.skip(8))?;
            Ok(Program { format, inner })
        }
    }
}
