...
pub trait Encode {
    ...
    fn encode(&self, w: &mut Vec<u8>) -> std::io::Result<()> {
        let meta = self.encode_body(w)?;
        self.encode_head(meta, w)?;
        Ok(())
    }

    fn encode_head(&self, body_meta: Self::BodyMeta, w: &mut Vec<u8>) -> std::io::Result<()>;

    fn encode_body(&self, w: &mut Vec<u8>) -> std::io::Result<Self::BodyMeta>;
}

impl Encode for i64 {
    type BodyMeta = ();

    fn encode_body(&self, _w: &mut Vec<u8>) -> std::io::Result<Self::BodyMeta> {
        Ok(())
    }

    fn encode_head(&self, _: (), w: &mut Vec<u8>) -> std::io::Result<()> {
...
impl Encode for String {
    ...
    fn encode_head(&self, bytes_start: usize, w: &mut Vec<u8>) -> std::io::Result<()> {
...
pub trait Decode: Layout + Sized {
    fn decode_bytes(bytes: &[u8]) -> std::io::Result<Self> {
    ...
    fn decode(r: &mut Reader<'_>) -> std::io::Result<Self>;
}
