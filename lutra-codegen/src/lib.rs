...
pub struct Context<'a> {
    ...
    types: &'a HashMap<ir::Path, &'a ir::Ty>,
}
...
fn codegen_main(
    ...
    let ty_defs: HashMap<_, _> = module
        .iter_decls_re()
        .filter_map(|(p, d)| {
            if let ir::Decl::Type(ty) = d {
                Some((p, ty))
            } else {
                None
            }
    ...
    let module_path = vec![];
    codegen_module(&mut w, &module, module_path, options, &ty_defs, root_mod)?;
    Ok(w)
}
...
fn codegen_module(
    ...
    let mut ctx = Context::new(module_path.clone(), options, ty_defs);
    ...
    for (name, sub_mod) in sub_modules {
        ...
        codegen_module(w, sub_mod, path, options, ty_defs, root_mod)?;
        writeln!(w, "}}\n")?;
    }
