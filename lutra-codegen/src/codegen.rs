...
pub fn codegen_types(module: &decl::Module) -> Result<String, std::fmt::Error> {
    let mut w = String::new();
    writeln!(w, "//# Generated by lutra-codegen\n")?;

    let mut ctx = Context::default();

    codegen_types_in(&mut w, module, &mut ctx)?;
    Ok(w)
}
...
fn codegen_types_in(
    ...
    for (name, sub_mod) in sub_modules {
        writeln!(w, "mod {name} {{")?;
        codegen_types_in(w, sub_mod, ctx)?;
        writeln!(w, "}}\n")?;
    }
